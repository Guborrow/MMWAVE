    ...
    while True:
        ...
        if frameNumber >= 1 and frameNumber > frameNumber_next:
            ...
            if numDetectedObj > 0 and dataOK == 2:
                ...
                pointObj_snr = np.reshape(20 * np.log10(side_info["snr"] + 1), (-1, 1))  # Signal-to-Noise Ratio

                pointObj = np.concatenate((pointObj_FrameID, pointObj_PointNum,pointObj_x, pointObj_y, pointObj_z,
                                            pointObj_velocity, pointObj_snr), axis=1)
                print(pointObj)
                if GetData_flag == 1:
                    min_SNR = np.min(pointObj[:, 6]) * 0.9
                    pre_pointObj[:, 6] = pre_pointObj[:, 6] * 0.7
                    pointObj = np.vstack((pointObj, pre_pointObj))

                    key_pointObj = np.where(pointObj[:, 6] >= min_SNR)[0]
                    pointObj = pointObj[key_pointObj, :]
                    pointObj[:, 0] = np.ones(len(key_pointObj), dtype=int) * frameNumber
                    pointObj[:, 1] = np.ones(len(key_pointObj), dtype=int) * len(key_pointObj)
                GetData_flag = 1
                pre_pointObj = pointObj
